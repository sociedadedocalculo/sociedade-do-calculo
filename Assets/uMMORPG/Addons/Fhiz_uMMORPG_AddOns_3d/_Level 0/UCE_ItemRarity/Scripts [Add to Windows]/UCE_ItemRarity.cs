        ...
        if (player != null)
            switch (slotType)
            {
                ...
                case SlotType.Loot:
                    ...
                    {
                        int lastLCount = 0;
                        if (lastLCount != player.target.inventory.Count)
                            for (int i = 0; i < player.target.inventory.Count; ++i)
                            {
                                lastLCount = player.target.inventory.Count;
                                if (player.target.inventory[i].amount > 0)
                                {
                                    UILoot lootContents = GetComponent<UILoot>();
                                    List<ItemSlot> items = player.target.inventory.Where(slot => slot.amount > 0).ToList();
                                    UIUtils.BalancePrefabs(lootContents.itemSlotPrefab.gameObject, items.Count, lootContents.content);
                                    if (lootContents.panel.activeSelf)
                                    {
                                        UILootSlot slot = lootContents.content.transform.GetChild(i).GetComponent<UILootSlot>();
                                        raritySlot = slot.GetComponent<UCE_RaritySlot>();
                                        tooltip = slot.tooltip;
                                        slot.dragAndDropable.name = i.ToString(); // drag and drop slot
                                        itemSlot = player.target.inventory[i];
                                        SetRarityColor(itemSlot.item.data);
                                    }
                                }
                                else
                                {
                                    UILoot lootContents = gameObject.GetComponent<UILoot>();
                                    if (lootContents.panel.activeSelf)
                                    {
                                        UILootSlot slot = lootContents.content.transform.GetChild(i).GetComponent<UILootSlot>();
                                        raritySlot = slot.GetComponent<UCE_RaritySlot>();
